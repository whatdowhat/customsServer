...
public class FileUploadSpecification {
	...
	public static Specification<FileUpload> withCondition(Map<String, String> condition) {

		return (Specification<FileUpload>) ((root, query, builder) -> {
			List<Predicate> predicate = getPredicateWithKeyword(condition, root, builder);
			return builder.and(predicate.toArray(new Predicate[0]));
		});
	}

	private static List<Predicate> getPredicateWithKeyword(Map<String, String> conditino, Root<FileUpload> root,
			CriteriaBuilder builder) {
		List<Predicate> predicate = new ArrayList<>();
		for (String key : conditino.keySet()) {
			if ("inboundMasterId".equals(key)) {
				if (conditino.get(key) == null || conditino.get(key).equals("")|| conditino.get(key).equals(0) ) {
				} else {
