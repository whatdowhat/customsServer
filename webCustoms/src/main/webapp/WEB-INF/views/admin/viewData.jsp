...
<!-- 			<ol class="breadcrumb float-xl-end">
			...
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			<h1 class="page-header">매칭하기 <!-- <small>header small text goes here...</small> --></h1>
			<!-- END page-header -->
			<!-- BEGIN row -->
 ...
 <br>
								...
								<select name="searchColum" id="searchColum">
 								...
 								<button type="button" class="btn btn-primary" id="deleteSeq" onclick="deleteSeq()">데이터 삭제</button>
...
function init(){
	...
	$.ajax.PromisePostForm('/admin/viewData/selectAllProjectAndTeam?${_csrf.parameterName}=${_csrf.token}',formData)
	.then(showTree);
...
function EDIT_LOG(origin,edit){
    ...
    $.ajax({
    	url: '/admin/viewData/edit_new?${_csrf.parameterName}=${_csrf.token}',
...
function click_confirm(param){
	...
	$.ajax({
  	url: '/admin/viewData/confirm?${_csrf.parameterName}=${_csrf.token}',
      data:data,
      type: 'post',
      success:function(data){ //"inedx3"
...
function click_confirmFinal(param){
	 //console.log(param);
	var info = _table1.row(param).data();
	 //console.log(info.seq);
	//console.log(info);
	...
	$.ajax({
 	url: '/admin/viewData/confirmFinal?${_csrf.parameterName}=${_csrf.token}',
     data:data,
     type: 'post',
     success:function(data){ //"inedx3"
...
function callDataClickTeam(data){
	...
	return 	 $.ajax({
    	url: '/admin/viewData/selectTREE?${_csrf.parameterName}=${_csrf.token}',
    	data: data,
        type: 'post',
        ...
        _table1=$('#datatable').DataTable({
	    	...
	    	"columns" : [ 
	        	...
	        	{ data: 'seq',render:function(data, type, full, meta){
	        		return `<input type="checkbox" name=seqCheck  value=`+data+` onclick="event.cancelBubble=true" />`; 
				}},
	        	{ data: 'seq',render:function(data, type, full, meta){
			    ...
			    { data: 'confirmFinal', render:function(data, type, full, meta){
	        	...
	        	{ data: 'confirmFinal',render:function(data, type, full, meta){
			    	return '<button class="btn btn-danger btn-lg" value="'+data+'" id="btn_'+data+'" onclick="click_confirmFinal('+meta.row+')" />확정하기</button>';
				}}
...
function callDataClickProject(data){
	...
	return 			 $.ajax({
    	url: '/admin/viewData/selectProjectTREE?${_csrf.parameterName}=${_csrf.token}',
    	data: data,
        type: 'post',
        success:function(data){ //"inedx3"
