	...
	public List<ChinaSanggumRes> getChinaSanggum(ChinaSanggumReq chinaSanggumReq)  {
		...
		for(int i=0; i<finalInboundList.size(); i++) {
			...
			for(int j=0; j<ColorType.getList().size();j++) {
				if(ColorType.getList().get(j).getId() ==finalInboundList.get(i).getCorpId() ) {
					ch.setCorpType(ColorType.getList().get(j).getShowName());
					
				}
			}
			ch.setIncomeDtStr(finalInboundList.get(i).getIncomeDt());
			ch.setRegDtStr(format.format(target.getRegDate()));
			ch.setMemo(target.getMemo()==null?"":target.getMemo());
